...
class GoodsBatchController extends InitController
{
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'add') {
            ...
            $download_list = [];
            foreach ($dir as $file) {
                if ($file != '.' && $file != '..' && $file != ".svn" && $file != "_svn" && is_dir(resource_path('lang/' . $file)) == true) {
            ...
            return $this->smarty->display('goods_batch_add.htm');
        }
                    ...
                    foreach ($_POST['goods_id'] as $goods_id) {
                        ...
                        if (!empty($_POST['product_number'][$goods_id])) {
                            ...
                            foreach ($_POST['product_number'][$goods_id] as $key => $value) {
                                $this->db->autoExecute($this->ecs->table('products'), ['product_number', $value], 'UPDATE', "goods_id = '$goods_id' AND product_id = " . $key);
