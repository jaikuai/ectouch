...
class AffiliateCkController extends InitController
{
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            if (!empty($_GET['auid'])) {
                $this->smarty->assign('action_link', ['text' => $GLOBALS['_LANG']['back_note'], 'href' => "users.php?act=edit&id=$_GET[auid]"]);
            }

            return $this->smarty->display('affiliate_ck_list.htm');
        }
        ...
        if ($_REQUEST['act'] == 'query') {
            ...
            return make_json_result($this->smarty->fetch('affiliate_ck_list.htm'), '', ['filter' => $logdb['filter'], 'page_count' => $logdb['page_count']]);
        }
