...
//    $this->smarty->assign('back_act', $back_act);
        ...
        if ($action == 'act_edit_password') {
            ...
            if (($user_info && (!empty($code) && md5($user_info['user_id'] . $GLOBALS['_CFG']['hash_code'] . $user_info['reg_time']) == $code)) || (session('user_id') > 0 && session('user_id') == $user_id && $this->user->check_user(session('user_name'), $old_password))) {
                if ($this->user->edit_user(['username' => (empty($code) ? session('user_name') : $user_info['user_name']), 'old_password' => $old_password, 'password' => $new_password], empty($code) ? 0 : 1)) {
        ...
        if ($action == 'act_add_bonus') {
            ...
            if (add_bonus($user_id, $bouns_sn)) {
                ...
                return $this->err->show($GLOBALS['_LANG']['back_up_page'], 'user.php?act=bonus');
            }
        }
        ...
        if ($action == 'act_add_message') {
            ...
            if (add_message($message)) {
                ...
                return $this->err->show($GLOBALS['_LANG']['message_list_lnk'], 'user.php?act=message_list');
            }
        }
        ...
        if ($action == 'merge_order') {
            ...
            if (merge_user_order($from_order, $to_order, $user_id)) {
                ...
                return $this->err->show($GLOBALS['_LANG']['order_list_lnk']);
            }
        }
        ...
        if ($action == 'act_transform_points') {
            ...
            switch ($rule_key) {
                case TO_P:
                    ...
                    $this->user->set_points($row['user_name'], [$bbs_key => 0 - $num]); //调整论坛积分
                ...
                case FROM_P:
                    ...
                    $this->user->set_points($row['user_name'], [$bbs_key => $result_points]); //调整论坛积分
