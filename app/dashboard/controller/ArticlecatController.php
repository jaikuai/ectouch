...
class ArticlecatController extends InitController
{
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            return $this->smarty->display('articlecat_list.htm');
        }
        ...
        if ($_REQUEST['act'] == 'query') {
            ...
            return make_json_result($this->smarty->fetch('articlecat_list.htm'));
        }
        ...
        if ($_REQUEST['act'] == 'add') {
            ...
            return $this->smarty->display('articlecat_info.htm');
        }
        ...
        if ($_REQUEST['act'] == 'insert') {
           ...
           VALUES ('$_POST[cat_name]', '$cat_type',  '$_POST[cat_desc]','$_POST[keywords]', '$_POST[parent_id]', '$_POST[sort_order]', '$_POST[show_in_nav]')";
            ...
            if ($_POST['show_in_nav'] == 1) {
                ...
                $vieworder += 2;
                //显示在自定义导航栏中
                $sql = "INSERT INTO " . $this->ecs->table('nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type) VALUES('" . $_POST['cat_name'] . "', 'a', '" . $this->db->insert_id() . "','1','$vieworder','0', '" . build_uri('article_cat', ['acid' => $this->db->insert_id()], $_POST['cat_name']) . "','middle')";
                $this->db->query($sql);
            }
